...
contract D36_MinEIP712Verifier {
    ...
    // --- Typed data: Mail ---
    // 注意：字段顺序必须严格一致
    bytes32 public constant MAIL_TYPEHASH = keccak256("Mail(address to,uint256 amount,uint256 nonce,uint256 deadline)");
    ...
    constructor(string memory name, string memory version) {
        ...
        DOMAIN_SEPARATOR =
            keccak256(abi.encode(EIP712DOMAIN_TYPEHASH, NAME_HASH, VERSION_HASH, block.chainid, address(this)));
    }
    ...
    function verify(address expectedSigner, Mail memory m, uint8 v, bytes32 r, bytes32 s) public view returns (bool) {
